...
@app.route("/")
def home():
  return "Quant Trading Bot Engine is Active & Running!"
...
def keep_alive():
  """ایجاد تاپیک جداگانه برای وب‌سرور Flask جهت پاسخگویی به پورت Render"""
...
class AdvancedOrderBookAnalyzer:
  ...
  def analyze(
    ...
    stop_hunt_risk = self._detect_stop_hunt(
        clean_bids, clean_asks, current_price, atr
    )
  ...
  def _estimate_slippage(
      self, order_side: List[Tuple[float, float]], amount_usd: float
  ) -> float:
    if not order_side:
      return 999.0
  ...
  def _detect_walls(
      self, order_side: List[Tuple[float, float]]
  ...
  def _detect_icebergs(self, order_side: List[Tuple[float, float]]) -> int:
    ...
    iceberg_count = sum(
        1 for _, qty in slice_depth if 1.8 * avg_qty <= qty <= 3.0 * avg_qty
    )
  ...
  def _detect_stop_hunt(
    ...
    near_bid_vol = sum(
        p * v for p, v in bids if abs(current_price - p) <= 1.2 * atr
    )
    near_ask_vol = sum(
        p * v for p, v in asks if abs(current_price - p) <= 1.2 * atr
    )
...
async def fetch_advanced_order_book(
    session: aiohttp.ClientSession,
    symbol: str,
    depth_limit: int = 100,
    atr: float = 0.0,
) -> Optional[AdvancedOBMetrics]:
  ...
  exchanges = [
      {
          "name": "Binance",
          "url": (
              f"https://fapi.binance.com/fapi/v1/depth?symbol={clean_symbol}&limit={depth_limit}"
          ),
      ...
      },
      {
          "name": "Bybit",
          "url": (
              "https://api.bybit.com/v5/market/orderbook?category=linear&symbol="
              f"{clean_symbol}&limit=50"
          ),
          "parser": lambda d: (
              d.get("result", {}).get("b", []),
              d.get("result", {}).get("a", []),
      ...
      },
      {
          ...
          "url": (
              ...
              f"{clean_symbol[:3]}-USDT-SWAP&sz=100"
          ),
          "parser": lambda d: (
              d.get("data", [{}])[0].get("bids", []),
              d.get("data", [{}])[0].get("asks", []),
          ),
      },
  ]
  ...
  for ex in exchanges:
    try:
      async with session.get(
          ex["url"], timeout=aiohttp.ClientTimeout(total=4)
      ) as resp:
        if resp.status == 200:
      ...
      LOGGER.debug(
          f"OrderBook fetch failover [{ex['name']}] for {symbol}: {e}"
      )
      continue
...
async def fetch_klines_with_failover(
    session: aiohttp.ClientSession,
    symbol: str,
    interval: str = "15m",
    limit: int = 100,
  ...
  exchanges = [
      {
          "name": "Binance",
          "url": (
              f"https://fapi.binance.com/fapi/v1/klines?symbol={clean_symbol}&interval={interval}&limit={limit}"
          ),
          "parser": lambda data: [
              {
                  "time": int(k[0]),
      ...
      },
      {
          ...
          "url": (
              ...
              f"{clean_symbol}&interval=15&limit={limit}"
          ),
          "parser": lambda data: [
              {
                  "time": int(k[0]),
  ...
  for ex in exchanges:
    try:
      async with session.get(
          ex["url"], timeout=aiohttp.ClientTimeout(total=5)
      ) as resp:
        if resp.status == 200:
          data = await resp.json()
...
def generate_technical_signal(
    symbol: str, features: Dict[str, float]
) -> Optional[Signal]:
  ...
  if ema20 > ema50 and 52 < rsi < 70:
    ...
    tp2 = close + (3.0 * atr)
    tp3 = close + (4.5 * atr)
    ai_score = min(95.0, round(75.0 + abs(rsi - 50) * 0.8, 1))
    ...
    tp2 = close - (3.0 * atr)
    tp3 = close - (4.5 * atr)
    ai_score = min(95.0, round(75.0 + abs(50 - rsi) * 0.8, 1))
...
def advanced_ob_filter(
    signal: Signal, ob: Optional[AdvancedOBMetrics]
) -> Tuple[bool, str]:
  if not ob:
  ...
  if signal.direction == "BUY":
    if ob.liquidity_imbalance < -0.35:
      return (
          False,
          f"Rejected BUY: High Sell Liquidity Imbalance ({ob.liquidity_imbalance:.2f})",
      )

    if ob.ask_slippage_10k > 0.8:
      return (
          False,
          f"Rejected BUY: Excessive Ask Slippage ({ob.ask_slippage_10k:.2f}%)",
      )

    if ob.stop_hunt_risk > 0.85:
      return (
          False,
          f"Rejected BUY: Critical Stop Hunt Risk ({ob.stop_hunt_risk:.2f})",
      )
    ...
    if ob.liquidity_imbalance > 0.35:
      return (
          False,
          f"Rejected SELL: High Buy Liquidity Imbalance ({ob.liquidity_imbalance:.2f})",
      )

    if ob.bid_slippage_10k > 0.8:
      return (
          False,
          f"Rejected SELL: Excessive Bid Slippage ({ob.bid_slippage_10k:.2f}%)",
      )

    if ob.stop_hunt_risk > 0.85:
      return (
          False,
          f"Rejected SELL: Critical Stop Hunt Risk ({ob.stop_hunt_risk:.2f})",
      )
...
def format_telegram_signal(
  ...
  """ساخت پیام شکیل تلگرام همراه با هوش مصنوعی، سمبل‌ها، TPها و داده‌های کوانت"""
  dir_emoji = (
      "🟢 LONG / BUY"
  ...
  sl_percent = (
      abs((signal.stop_loss - signal.entry_price) / signal.entry_price) * 100
      if signal.entry_price
      else 0.0
  )

  imb = ob.liquidity_imbalance if ob else 0.0
  slip = (
      (ob.ask_slippage_10k if signal.direction == "BUY" else ob.bid_slippage_10k)
      if ob
      else 0.0
  )
  sh_risk = ob.stop_hunt_risk if ob else 0.0
  bids_ice = ob.iceberg_bids if ob else 0
  asks_ice = ob.iceberg_asks if ob else 0

  return (
      f"🚀 <b>QUANT & AI TRADING SIGNAL: #{signal.symbol}</b> 🚀\n\n"
      f"{dir_emoji}\n"
      f"🤖 <b>AI Confidence Score:</b> {signal.ai_score}%\n"
      f"⏱ <b>Timeframe:</b> 15m\n\n"
      f"💵 <b>Entry Price:</b> ${signal.entry_price:.4f}\n"
      f"🛑 <b>Stop Loss:</b> ${signal.stop_loss:.4f} (-{sl_percent:.2f}%)\n\n"
      f"🎯 <b>TP1:</b> ${signal.take_profit:.4f}\n"
      f"🚀 <b>TP2:</b> ${signal.take_profit_2:.4f}\n"
      f"🔥 <b>TP3:</b> ${signal.take_profit_3:.4f}\n\n"
      f"💡 <b>Reason:</b> {signal.reason}\n\n"
      f"──────────────────────\n"
      f"📊 <b>Order Book Microstructure (Quant):</b>\n"
      f"⚖️ <b>Imbalance Ratio:</b> {imb:.2f}\n"
      f"💧 <b>10k Slippage:</b> {slip:.2f}%\n"
      f"🛡 <b>Stop Hunt Risk:</b> {sh_risk:.2f}\n"
      f"🧊 <b>Iceberg Orders (Bids/Asks):</b> {bids_ice} / {asks_ice}\n"
      f"──────────────────────\n"
      f"⚡️ <i>Trading Bot Quant Engine v2.0</i>"
  )
...
async def process_single_symbol(
  ...
  klines = await fetch_klines_with_failover(
      session, symbol, interval="15m", limit=100
  )
  if not klines:
    return
  ...
  ob_metrics = await fetch_advanced_order_book(
      session, symbol, depth_limit=100, atr=current_atr
  )
  ...
  if is_valid:
    ...
    msg = format_telegram_signal(raw_signal, ob_metrics)
    ...
    await execute_db_query_async(
        "INSERT INTO signals (symbol, direction) VALUES (?, ?)",
        (symbol, raw_signal.direction),
    )
  else:
...
async def main():
  ...
  async with aiohttp.ClientSession() as session:
    LOGGER.info("--- Initializing Continuous Market Scan Engine ---")
    # حلقه بی‌نهایت جهت جلوگیری از خاموش شدن برنامه (Application exited early)
    while True:
      try:
        LOGGER.info("Scanning market for signals...")
        tasks = [
            process_single_symbol(
                session, symbol, rate_limiter, deduplicator, telegram
            )
      ...
      # هر ۵ دقیقه (۳۰۰ ثانیه) اسکن بعدی انجام می‌شود
      await asyncio.sleep(300)


if __name__ == "__main__":
  # اجرای وب‌سرور برای زنده نگه‌داشتن پورت روی Render
  keep_alive()
